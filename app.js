...
async function init(){
 ...
 document.querySelectorAll('[data-page]').forEach(e=>e.addEventListener('click',ev=>{ev.preventDefault();go(e.dataset.page);document.querySelector('.sidebar')?.classList.remove('open')}));
 $('sidebar-toggle')?.addEventListener('click',()=>{document.querySelector('.sidebar')?.classList.toggle('open')});
 $('client-notif-btn')?.addEventListener('click',()=>go('client-notifications'));
...
async function renderClient(){await loadZones();const orders=(await DB.all('orders')).filter(x=>x.clientId===currentUser.id);$('client-name').textContent=currentUser.name;if(currentUser.avatar)$('client-avatar').src=currentUser.avatar;$('client-completed').textContent=orders.filter(x=>x.status==='completed').length;$('client-active').textContent=orders.filter(x=>!['completed','cancelled'].includes(x.status)).length;$('client-cancelled').textContent=orders.filter(x=>x.status==='cancelled').length;$('client-recent-orders').innerHTML=orderCards(orders.slice(-5).reverse(),'client');const cur=orders.find(x=>!['completed','cancelled'].includes(x.status));$('client-current-order').innerHTML=cur?orderCard(cur,'client'):empty('لا يوجد طلب جاري حالياً');window._clientOrdersAll=orders.reverse();renderClientOrdersList('all');await renderNotifications();await renderRanking()}
async function renderRanking(){
  const now=new Date(), month=`${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,'0')}`;
  const orders=await DB.all('orders'), users=await DB.all('users');
  const clients=users.filter(u=>u.role==='client'&&u.status==='active');
  const stats=clients.map(u=>{const os=orders.filter(o=>o.clientId===u.id&&o.status==='completed'&&String(o.completedAt||o.updatedAt||o.createdAt).slice(0,7)===month);return {...u,monthlyOrders:os.length,monthlySpent:os.reduce((s,o)=>s+Number(o.cost||0),0)}}).sort((a,b)=>b.monthlyOrders-a.monthlyOrders||b.monthlySpent-a.monthlySpent);
  const top=stats.slice(0,10); const mine=stats.find(x=>x.id===currentUser?.id);
  const rewards=await DB.all('ranking_rewards');
  const rewardByUser={}; rewards.filter(x=>x.month===month).forEach(x=>rewardByUser[x.userId]=x);
  const box=$('client-ranking-list'); if(box) box.innerHTML=top.length?top.map((u,i)=>`<div class="ranking-row ${u.id===currentUser?.id?'me':''}"><div class="rank-medal">${i<3?['🥇','🥈','🥉'][i]:i+1}</div><div class="ranking-avatar"><img src="${u.avatar||'https://ui-avatars.com/api/?name='+encodeURIComponent(u.name)+'&background=0f766e&color=fff'}"></div><div class="ranking-user"><b>${esc(u.name)}</b><small>${u.monthlyOrders} طلب مكتمل · ${money(u.monthlySpent)} ج.م</small></div>${rewardByUser[u.id]?'<span class="reward-badge"><i class="fas fa-gift"></i> تم استلام الهدية</span>':''}</div>`).join(''):empty('لا توجد بيانات لهذا الشهر');
  if($('client-ranking-month'))$('client-ranking-month').textContent=month;
  if($('client-ranking-me'))$('client-ranking-me').innerHTML=mine?`ترتيبك هذا الشهر: <b>#${stats.findIndex(x=>x.id===mine.id)+1}</b> · ${mine.monthlyOrders} طلب مكتمل`:'ابدأ باستخدام التطبيق للدخول في الترتيب';
}
async function renderAdminRanking(){
  const now=new Date(), month=`${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,'0')}`;
  const orders=await DB.all('orders'), users=await DB.all('users'), rewards=await DB.all('ranking_rewards');
  const clients=users.filter(u=>u.role==='client');
  const rows=clients.map(u=>{const os=orders.filter(o=>o.clientId===u.id&&o.status==='completed'&&String(o.completedAt||o.updatedAt||o.createdAt).slice(0,7)===month);return {u,count:os.length,spent:os.reduce((s,o)=>s+Number(o.cost||0),0)}}).filter(x=>x.count>0).sort((a,b)=>b.count-a.count||b.spent-a.spent);
  const box=$('admin-ranking-table'); if(!box)return;
  box.innerHTML=rows.length?`<table><thead><tr><th>#</th><th>العميل</th><th>الطلبات المكتملة</th><th>الإنفاق</th><th>الهدية</th><th>إجراء</th></tr></thead><tbody>${rows.map((x,i)=>{const r=rewards.find(r=>r.month===month&&r.userId===x.u.id);return `<tr><td>${i+1}</td><td><b>${esc(x.u.name)}</b><br><small>${esc(x.u.phone)}</small></td><td>${x.count}</td><td>${money(x.spent)} ج.م</td><td>${r?`<span class="reward-badge"><i class="fas fa-gift"></i> ${esc(r.reward||'هدية شهرية')}</span>`:'—'}</td><td><button class="btn btn-sm ${r?'btn-outline':'btn-primary'}" onclick="window.grantRankingReward('${x.u.id}','${month}')" ${r?'disabled':''}><i class="fas fa-gift"></i> ${r?'تم التسجيل':'منح الهدية'}</button></td></tr>`}).join('')}</tbody></table>`:empty('لا توجد بيانات استخدام مكتملة لهذا الشهر');
  if($('admin-ranking-month'))$('admin-ranking-month').textContent=month;
}
window.grantRankingReward=async(id,month)=>{const u=await DB.get('users',id);if(!u)return;const reward=prompt('اكتب اسم الهدية الشهرية للعميل:', 'هدية شهرية');if(reward===null)return;const r={id:`${month}_${id}`,month,userId:id,userName:u.name,reward:reward||'هدية شهرية',grantedBy:currentUser.id,createdAt:DB.now()};await DB.put('ranking_rewards',r);await DB.notify(id,'مبروك! 🎁','حصلت على هدية الشهر بسبب نشاطك في التطبيق: '+r.reward,'success',{month,reward:r.reward});await DB.log(currentUser.id,'grant_ranking_reward','منح هدية شهرية',{userId:id,month,reward:r.reward});toast('تم تسجيل الهدية وإرسال إشعار للعميل');renderAdminRanking();};
...
async function setupRealtime(){
  ...
  const rerender=()=>{ if(currentUser.role==='admin') { refresh(); if($('admin-ranking')?.classList.contains('active')) renderAdminRanking(); } else refresh(); renderSupportIfOpen(); };
  ['users','orders','zones','settings','notifications','ranking_rewards'].forEach(col=>realtimeUnsubs.push(DB.listen(col,()=>rerender(),()=>{})));
